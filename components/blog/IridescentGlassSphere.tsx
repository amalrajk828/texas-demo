...
export interface IridescentGlassSphereProps {
  className?: string;
...
float bubbleNoise(vec3 p, float t) {
  // Low-frequency breathing and gentle elastic bulges
...
// Violet-magenta-indigo thin-film iridescence (strictly tuned to site theme, no green/yellow rainbow)
vec3 thinFilmColor(float cosTheta, vec3 p, float t) {
  // Film thickness variation drifting across the fluid surface
  float thickness = 0.52 + 0.38 * sin(p.x * 1.8 + p.y * 1.4 + p.z * 1.2 + t * 0.22);
  float opd = 2.0 * 1.38 * thickness * cosTheta; // Optical Path Difference

  float phi = opd * 4.2 + t * 0.12;

  // Curated violet-magenta palette matching --accent oklch(0.7 0.19 300)
  vec3 deepViolet  = vec3(0.42, 0.12, 0.82); // Deep rich violet
  vec3 brightMag   = vec3(0.86, 0.24, 0.76); // Vibrant magenta accent
  vec3 royalIndigo = vec3(0.18, 0.07, 0.48); // Deep indigo base
  vec3 softPink    = vec3(0.96, 0.65, 0.90); // Soft pastel magenta sheen
  vec3 pearlescent = vec3(0.98, 0.94, 1.00); // Near-white iridescent flash

  float w1 = sin(phi) * 0.5 + 0.5;
  float w2 = cos(phi * 0.85 + 1.4) * 0.5 + 0.5;
  float w3 = sin(phi * 1.6 + 2.8) * 0.5 + 0.5;

  vec3 col = mix(royalIndigo, deepViolet, w1);
  col = mix(col, brightMag, w2 * 0.85);
  col = mix(col, softPink, w3 * 0.45);
  col = mix(col, pearlescent, pow(w3, 4.0) * 0.7);
...
vec3 studioEnvironment(vec3 rayDir, float t) {
  // Near-black violet background (#0a0a0f)
  vec3 bgDark = vec3(0.039, 0.039, 0.059);
  vec3 bgVioletGlow = vec3(0.14, 0.06, 0.26);

  float up = rayDir.y * 0.5 + 0.5;
  vec3 bg = mix(bgDark, bgVioletGlow, pow(up, 2.5) * 0.4);

  // Soft violet studio rim bounce
  float leftRim = smoothstep(0.2, -0.8, rayDir.x);
  vec3 rimTone = vec3(0.55, 0.15, 0.85) * leftRim * 0.35;
...
void main() {
  ...
  // Background deep near-black violet (#0a0a0f) with subtle radial atmospheric glow
  vec3 bgBase = vec3(0.039, 0.039, 0.059);
  float distToCenter = length(uv);
  float aura = exp(-distToCenter * 2.2);
  vec3 auraGlow = vec3(0.48, 0.15, 0.82) * aura * 0.32;
  ...
  if (hit) {
    ...
    float f0 = 0.04; // typical glass/water
    float fresnel = f0 + (1.0 - f0) * pow(1.0 - cosTheta, 3.2);

    // Dynamic thin-film iridescence on the bubble surface
    ...
    vec3 warmTone = vec3(1.0, 0.94, 0.85); // Warm pearlescent highlight

    glassBody += warmTone * specWarmSoft * 0.45;
    glassBody += vec3(1.0, 0.98, 0.94) * specWarmSharp * 1.15;

    // Secondary Violet/Magenta Key Light for vibrant rim punch
    ...
    glassBody += vec3(0.85, 0.25, 0.95) * specRim * 0.75;
    ...
    glassBody += vec3(0.28, 0.08, 0.52) * innerGlow * 0.3;
    ...
    col += vec3(0.55, 0.18, 0.85) * halo * 0.22;
  }

  // Tone mapping and gamma correction
  col = col / (col + vec3(0.85)); // Reinhard tone mapping
  col = pow(col, vec3(1.0 / 1.15)); // Gamma curve

  gl_FragColor = vec4(col, 1.0);
}
`;

export default function IridescentGlassSphere({
  className = "",
  overlayOpacity = 0.0,
}: IridescentGlassSphereProps) {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn("WebGL not supported for IridescentGlassSphere.");
      return;
    }
