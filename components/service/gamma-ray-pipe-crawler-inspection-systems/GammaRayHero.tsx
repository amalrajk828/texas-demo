...
import { RadioTower, ChevronRight, ArrowRight } from "lucide-react";

const STATS = [
  { value: "X-Ray / Gamma", label: "Radiation Source" },
  { value: "SWSI", label: "Single Wall Single Image" },
  { value: "Circumferential Welds", label: "Primary Target" },
  { value: "Kuwait & Dubai", label: "Our Locations" },
];

export default function GammaRayHero() {
  return (
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        <div className="fade-up">
          <nav className="flex items-center gap-2 text-[14px] text-white/50 tracking-[1.5px] uppercase mb-10 flex-wrap">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white/60">Gamma Ray Pipe Crawler</span>
          </nav>
        </div>
        <div className="max-w-full">
          <div className="relative rounded-2xl overflow-hidden aspect-[16/7] mb-10 shadow-2xl shadow-black/40">
            <Image src="/images/services/jme-her-pipeline-crawler-buggy-controller.webp" alt="Gamma ray pipe crawler inspection system for pipeline welds — TTS Kuwait" fill sizes="100vw" priority quality={80} className="object-cover opacity-60" />
          ...
          <div className="fade-up d1">
            <div className="inline-flex items-center gap-2.5 bg-[#e7212b]/10 border border-[#e7212b]/20 rounded-full px-4 py-2 mb-6">
              <RadioTower className="w-3.5 h-3.5 text-[#e7212b]" strokeWidth={2} />
          ...
          <div className="fade-up d1">
            <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] font-bold text-white leading-[1.08] tracking-tight mb-6">
              Gamma Ray Pipe{" "}
              <span className="text-white">Crawler Inspection</span>
            </h1>
          </div>
          <div className="fade-up d2">
            <p className="text-white/70 text-lg leading-relaxed max-w-none mb-10">
              The X-ray/gamma ray pipe crawler is similar to conventional radiography — a battery-powered, remote-controlled mobile radiographic machine designed to produce single wall single image (SWSI) radiographs of circumferential welds in pipelines, enabling internal weld inspection from within the pipe.
            </p>
          </div>
          <div className="fade-up d2">
