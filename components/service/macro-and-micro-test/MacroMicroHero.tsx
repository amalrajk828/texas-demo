...
const STATS = [
  { value: "Macro", label: "Weld Section Examination" },
  { value: "Micro", label: "Grain Structure Analysis" },
  { value: "Multi-Scale", label: "Length Scale Testing" },
  { value: "QC & Design", label: "Applications" },
];

export default function MacroMicroHero() {
  return (
    <section className="relative bg-[#000000] blueprint-grid blueprint-dot-grid overflow-hidden pt-32 pb-16 lg:pb-20 min-h-[400px]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        <nav className="flex items-center gap-2 mb-8 text-[14px] text-white/50">
          ...
          <Link href="/service/mechanical-testing/" className="hover:text-white transition-colors">Mechanical Testing</Link><ChevronRight className="w-3.5 h-3.5 text-white/30" />
          <span className="text-white/50">Macro and Micro Test</span>
        </nav>
        <div className="max-w-full">
          <div className="flex items-center gap-2.5 mb-5">
            <div className="w-8 h-8 rounded-lg bg-[#e7212b]/15 border border-[#e7212b]/25 flex items-center justify-center">
              <Microscope className="w-4 h-4 text-[#e7212b]" strokeWidth={1.8} />
            </div>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-[3.4rem] font-bold text-white leading-[1.08] mb-6">
            Macro &amp; <em className="not-italic text-white">Micro Test</em>
          </h1>
          <p className="text-white/70 text-[16px] leading-[1.85] mb-10 max-w-none">
            Two categories of experiments conducted to study mechanical properties at different length scales — providing insights into material behaviour under various loading conditions for characterisation, design optimisation, and quality control across all industries.
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/service/mechanical-testing/" className="inline-flex items-center gap-2 border border-white/15 hover:border-white/30 text-white/60 hover:text-white px-7 py-3.5 rounded-lg transition-all duration-200 text-[14px]">← Mechanical Testing</Link>
          </div>
        </div>
