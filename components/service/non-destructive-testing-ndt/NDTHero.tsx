...
const STATS = [
  { value: "5 Methods", label: "NDT Techniques" },
  { value: "VI / PT", label: "MT / RT / UT" },
  { value: "Qualified", label: "NDT Technicians" },
  { value: "Kuwait & Dubai", label: "Regional Coverage" },
];

export default function NDTHero() {
  return (
    <section className="relative bg-[#000000] blueprint-grid blueprint-dot-grid overflow-hidden pt-32 pb-16 lg:pb-20 min-h-[400px]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        <nav className="flex items-center gap-2 mb-8 text-[14px] text-white/50">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-white/30" />
          <Link href="/service/inspection-testing/" className="hover:text-white transition-colors">Inspection &amp; Testing</Link>
          <ChevronRight className="w-3.5 h-3.5 text-white/30" />
          <span className="text-white/50">Non Destructive Testing (NDT)</span>
        </nav>

        <div className="max-w-full">
          <div className="flex items-center gap-2.5 mb-5">
            <div className="w-8 h-8 rounded-lg bg-[#e7212b]/15 border border-[#e7212b]/25 flex items-center justify-center">
              <ScanLine className="w-4 h-4 text-[#e7212b]" strokeWidth={1.8} />
            </div>
            <span className="text-[#0891B2] text-[10px] font-semibold tracking-[3.5px] uppercase">Inspection &amp; Testing Services</span>
          </div>

          <h1 className="text-4xl sm:text-5xl lg:text-[3.4rem] font-bold text-white leading-[1.08] mb-6">
            Non Destructive{" "}
            <em className="not-italic text-white">Testing (NDT)</em>
          </h1>

          <p className="text-white/70 text-[16px] leading-[1.85] mb-10 max-w-none">
            TTS provides a full range of non-destructive examination services with qualified, experienced technicians and modern equipment — including Visual Inspection (VI), Liquid/Dye Penetrant (PT), Magnetic Particle (MT), Radiographic Testing (RT), and Ultrasonic Testing (UT).
          </p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/service/inspection-testing/" className="inline-flex items-center gap-2 border border-white/15 hover:border-white/30 text-white/60 hover:text-white px-7 py-3.5 rounded-lg transition-all duration-200 text-[14px]">← Inspection &amp; Testing</Link>
          </div>
        </div>
